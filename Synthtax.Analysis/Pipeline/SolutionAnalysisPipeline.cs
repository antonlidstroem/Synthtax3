...
/// <summary>
/// Orchestrates a full analysis run. Loads the Roslyn workspace once and dispatches
/// all enabled analysis modules concurrently via a shared AnalysisContext.
/// </summary>
        ...
        if (options.IncludeCode)
            tasks.Add(RunSafe(
                async () => (_code as IContextAwareAnalysis) is { } ca
                    ? await ca.AnalyzeAsync(ctx, cancellationToken)
                    : (object)await _code.AnalyzeSolutionAsync(solutionPath, cancellationToken),
        ...
        if (options.IncludeSecurity)
            tasks.Add(RunSafe(
                async () => (_security as IContextAwareAnalysis) is { } ca
                    ? await ca.AnalyzeAsync(ctx, cancellationToken)
                    : (object)await _security.AnalyzeSolutionAsync(solutionPath, cancellationToken),
        ...
        if (options.IncludeMetrics)
            tasks.Add(RunSafe(
                async () => (_metrics as IContextAwareAnalysis) is { } ca
                    ? await ca.AnalyzeAsync(ctx, cancellationToken)
                    : (object)await _metrics.AnalyzeSolutionMetricsAsync(solutionPath, cancellationToken),
        ...
        if (options.IncludeCoupling)
            tasks.Add(RunSafe(
                async () => (_coupling as IContextAwareAnalysis) is { } ca
                    ? await ca.AnalyzeAsync(ctx, cancellationToken)
                    : (object)await _coupling.AnalyzeSolutionAsync(solutionPath, cancellationToken),
        ...
        if (options.IncludeAIDetection)
            tasks.Add(RunSafe(
                async () => (_ai as IContextAwareAnalysis) is { } ca
                    ? await ca.AnalyzeAsync(ctx, cancellationToken)
                    : (object)await _ai.AnalyzeSolutionAsync(solutionPath, cancellationToken),
