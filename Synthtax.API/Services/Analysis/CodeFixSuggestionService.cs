...
public static class CodeFixSuggestionService
{
    ...
    {
        // If the declaration has a side-effect RHS (method call), suggest assigning to discard
        ...
        {
            // Replace "var x = SomeCall()" with "_ = SomeCall()" or "await SomeCallAsync()"
            var replaced = System.Text.RegularExpressions.Regex.Replace(
                trimmed,
                @"(?:var|[A-Za-z\[\]<>?,\s]+)\s+" + System.Text.RegularExpressions.Regex.Escape(varName) + @"\s*=\s*",
                "_ = ");
            return $"// Option A – discard the result (if the call has side effects):\n{replaced}\n\n" +
                   $"// Option B – remove entirely if the call has no side effects.";
        }

        return $"// Remove this line entirely:\n// {trimmed}";
    }
        ...
        bool isInterpolation)
    {
        ...
        var fixedCode = GenerateSqlInjectionFix(originalCode, methodName);
        return (description, fixedCode, autoFixable: false);
    }

    private static string GenerateSqlInjectionFix(string originalCode, string methodName)
    {
        return $"""
// BEFORE (vulnerable):
{originalCode.Trim()}
...
""";
        ...
        List<string> innerCallsThatAcceptCt)
    {
        ...
        var fixedCode = GenerateCancellationTokenFix(methodSignature, methodName, innerCallsThatAcceptCt);
        return (description, fixedCode, autoFixable: false);
    }

    private static string GenerateCancellationTokenFix(
        string signature, string methodName, List<string> innerCalls)
    {
        // Insert CancellationToken parameter before closing paren
        string newSignature;
...
""";
        ...
        string codeSnippet)
    {
        ...
        var fixedCode = GenerateCognitiveComplexityAdvice(methodName, complexity, codeSnippet);
        return (description, fixedCode, autoFixable: false);
    }

    private static string GenerateCognitiveComplexityAdvice(
        string methodName, int complexity, string snippet)
    {
...
""";
        ...
        string callSite, string callerMethodName)
    {
        ...
        var fixed1 = callSite
            .Replace(".Result", " (await the Task instead)")
            .Replace(".Wait()", " (await the Task instead)");

        return (description,
            $"// BEFORE:\n{callSite}\n// AFTER: make the containing method async and await:\nawait ...",
            autoFixable: false);
    }
