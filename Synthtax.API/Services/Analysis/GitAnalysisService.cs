...
public class GitAnalysisService : IGitAnalysisService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using var repo = new Repository(repositoryPath);
            return GetCommitsInternal(repo, maxCommits);
        }, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using var repo = new Repository(repositoryPath);
            return GetBranchesInternal(repo);
        }, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using var repo = new Repository(repositoryPath);
            return GetFileChurnInternal(repo, maxCommits);
        }, cancellationToken);
    }
    ...
    private static List<GitCommitDto> GetCommitsInternal(Repository repo, int maxCommits)
    {
        ...
        return repo.Commits
            ...
            .Select(c =>
            {
                ...
                {
                    Sha = c.Sha,
                    ShortSha = c.Sha[..Math.Min(7, c.Sha.Length)],
                    Message = c.MessageShort.Trim(),
                    AuthorName = c.Author.Name,
                    AuthorEmail = c.Author.Email,
                    AuthoredAt = c.Author.When.UtcDateTime,
    ...
    private static (int filesChanged, int insertions, int deletions) GetCommitStats(
    Repository repo, Commit commit)
    {
        try
        {
            ...
            int insertions = patch.Sum(p => p.LinesAdded);
            int deletions = patch.Sum(p => p.LinesDeleted);
    ...
    private static List<BusFactorDto> GetBusFactorInternal(
        Repository repo, List<GitCommitDto> commits)
    {
        ...
        // Find which files each author primarily touched
        var filter = new CommitFilter
        {
            SortBy = CommitSortStrategies.Time,
            IncludeReachableFrom = repo.Head
