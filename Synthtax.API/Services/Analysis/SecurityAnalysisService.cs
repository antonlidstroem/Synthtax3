...
/// <summary>
/// Refactored to rule-engine pattern.
/// Rules: HardcodedCredential (SyntaxTree-based), SqlInjection (semantic),
///        InsecureRandom (improved context detection), MissingCancellationToken.
/// </summary>
public class SecurityAnalysisService : ISecurityAnalysisService, IContextAwareAnalysis
{
        ...
        AnalysisContext ctx, string solutionPath, CancellationToken ct)
    {
        ...
        try
        {
            var bags = _rules.ToDictionary(r => r.RuleId,
                _ => new ConcurrentBag<SecurityIssueDto>());
