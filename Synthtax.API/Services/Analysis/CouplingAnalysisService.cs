...
/// <summary>
/// Measures package coupling metrics as defined by Robert C. Martin:
///
///   Ca  – Afferent coupling: # types that depend ON this type
///   Ce  – Efferent coupling: # types this type depends on
///   I   – Instability = Ce / (Ca + Ce)   [0 = stable, 1 = unstable]
///   A   – Abstractness = abstract/interface members / total public members
///   D   – Distance from main sequence = |A + I - 1|   [0 = ideal]
///
/// Types with D > 0.5 are either "zone of pain" (concrete + stable) or
...
public class CouplingAnalysisService : ICouplingAnalysisService, IContextAwareAnalysis
{
        ...
        AnalysisContext ctx, string solutionPath, CancellationToken ct)
    {
        ...
        try
        {
            // ── Step 1: collect declared types and their semantic symbols ─────
            ...
            await Parallel.ForEachAsync(ctx.Documents,
                ...
                (doc, token) =>
                {
                    ...
                    {
                        if (typeDecl.Parent is TypeDeclarationSyntax) continue; // skip nested
            ...
            // ── Step 2: for each type, find what it references ────────────────
            ...
            await Parallel.ForEachAsync(ctx.Documents,
                ...
                (doc, token) =>
                {
                    ...
                    foreach (var typeDecl in root.DescendantNodes().OfType<TypeDeclarationSyntax>())
                    {
                        ...
                        // Collect all type references within this type
                        foreach (var id in typeDecl.DescendantNodes().OfType<IdentifierNameSyntax>())
                        {
                            ...
                            if (refFqn == ownerFqn) continue;          // self-reference
                            if (!typeNames.Contains(refFqn)) continue; // external type

                            lock (owner) owner.Efferents.Add(refFqn);

                            if (typeData.TryGetValue(refFqn, out var dependency))
                                lock (dependency) dependency.Afferents.Add(ownerFqn);
                        }
                    }
                    return ValueTask.CompletedTask;
                });

            // ── Step 3: compute metrics and build DTOs ────────────────────────
            foreach (var (fqn, data) in typeData)
            {
                ...
                result.Types.Add(dto);
            }

            result.Types.Sort((x, y) =>
                y.DistanceFromMainSequence.CompareTo(x.DistanceFromMainSequence));

            // ── Step 4: namespace-level aggregation ───────────────────────────
                ...
                result.Types
                    ...
                    .Select(g => new NamespaceCouplingDto
                    {
                        ...
                        OutgoingDependencies = g.SelectMany(t => t.DependsOn)
                            .Select(d => typeData.TryGetValue(d, out var td) ? td.Namespace : d)
                            .Distinct()
                            .Where(ns => ns != g.Key)
                            .ToList()
                    })
    ...
    private static double ComputeAbstractness(INamedTypeSymbol sym)
    {
        ...
        var abstractCount = publicMembers.Count(m =>
            m.IsAbstract || m.IsVirtual ||
            (m is IMethodSymbol ms && ms.IsAbstract));
    ...
    private static CouplingVerdict ClassifyType(int ca, int ce, double i, double d, INamedTypeSymbol sym)
    {
        ...
        // God class: many afferents AND many efferents
        if (ca + ce > 20 && ce > 10) return CouplingVerdict.GodClass;

        // Tightly coupled: too many efferents
        if (ce > 10) return CouplingVerdict.TightlyCoupled;

        // Unstable: high instability
