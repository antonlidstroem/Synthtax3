...
public partial class SecurityViewModel : AnalysisViewModelBase
{
    ...
    private void RefreshIssues()
    {
        CurrentIssues.Clear();
        if (_lastResult is null) return;
        var source = ShowAll ? _lastResult.AllIssues
            : ShowCredentials ? _lastResult.HardcodedCredentials
            : ShowSql         ? _lastResult.SqlInjectionRisks
            : ShowRandom      ? _lastResult.InsecureRandomUsage
            :                   _lastResult.MissingCancellationTokens;
